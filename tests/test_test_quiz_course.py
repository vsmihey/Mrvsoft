...
class TestLMS:
    ...
    TITLE_TEST = 'Тест без показа правильного варианта ответа ' + RAND_STR
    ...
    def test_create_new_test_1(self, driver):
        page = Exam(driver)
    ...
    def test_create_new_test_2(self, driver):
        page = Exam(driver)
        page.get_authorisation_in_superbank(user_for_test)
        page.create_button()
        page.create_test_button()
    ...
    @allure.title('Прохождение теста с ошибкой, ответ не верный')
    def test_passing_test_with_err(self, driver):
        page = Course(driver)
        page.get_authorisation_in_superbank(user_for_test)
        page.learn_button_click()
    ...
    @allure.title('Прохождение теста без ошибки, ответ верный')
